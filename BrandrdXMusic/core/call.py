...
[ SYSTEM: ULTIMATE HYBRID CORE ]
[ BASE: ALEXA (Queue Logic) + ANNIEX (Stream Engine) ]
[ STABILITY: 100% | SPEED: MAX ]
"""
...
# =======================================================================
# ⚙️ HYBRID ENGINE: ANNIE'S STREAMER + ALEXA'S LOGIC
# =======================================================================

def build_stream(path: str, video: bool = False, ffmpeg: str = None) -> MediaStream:
    """
    يستخدم تقنية AnnieX في بناء الستريم (إرسال معايير الفيديو دائماً)
    ولكن بذكاء Alexa في التعامل مع المسارات.
    """
    if video:
        return MediaStream(
            media_path=path,
        ...
        return MediaStream(
            media_path=path,
            audio_parameters=AudioQuality.HIGH,
            # الخدعة هنا: إرسال جودة فيديو 720p حتى لو صوت فقط لمنع الكراش
            video_parameters=VideoQuality.HD_720p, 
            audio_flags=MediaStream.Flags.REQUIRED,
            video_flags=MediaStream.Flags.IGNORE, # تجاهل الفيديو فعلياً
            ffmpeg_parameters=ffmpeg,
        )
...
class Call:
    ...
    async def start(self) -> None:
        LOGGER(__name__).info("🚀 Starting Hybrid Audio Engine (Alexa + Annie)...")
    ...
    async def decorators(self) -> None:
        ...
        async def unified_update_handler(client, update: Update) -> None:
            ...
            if isinstance(update, StreamEnded):
                # إذا انتهى الصوت، انتقل للمسار التالي (Alex Queue Logic)
        ...
        for assistant in assistants:
            try:
                # استخدام الطريقة الرسمية للمكتبة بدون تعديل
